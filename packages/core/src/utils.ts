...
export function toTitleFromPath(path: string): string {
...
export function scoreTextMatch(keyword: string, text: string): number {
...
export function snippetAroundKeyword(keyword: string, text: string, radius = 48): string {
  const normalizedKeyword = keyword.trim().toLowerCase();
  if (!normalizedKeyword) return text.slice(0, radius * 2);
